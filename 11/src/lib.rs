...
pub fn paint(program: &str, initial_color: Op) -> Panels {
    let mut panels: HashMap<Coord, Op> = HashMap::new();
    ...
    loop {
        ...
        computer.input(panel_color);

        let new_color = match computer.compute_output() {
            Some(WHITE) => WHITE,
            Some(BLACK) => BLACK,
            Some(_) => panic!("Unexpected color"),
            None => return panels,
        };

        if let Some(dir_change) = computer.compute_output() {
            dir = match dir_change {
                LEFT_90 => match dir {
                    Dir::Up => Dir::Left,
                    Dir::Right => Dir::Up,
                    Dir::Down => Dir::Right,
...
mod tests {
    ...
    #[test]
    fn eleven_1() {
        let panels = paint(PROGRAM_11, BLACK);
        assert_eq!(panels.len(), 2478);
    }

    #[test]
    fn eleven_2() {
        let panels = paint(PROGRAM_11, WHITE);
