...
fn resolve_op(ops: &Ops, index: usize, mode: OpMode) -> OpSize {
    match mode {
        OpMode::Immediate => ops[index],
        OpMode::Positional => ops[ops[index] as usize],
        OpMode::Relative => panic!("wrong impl"),
    }
}
...
impl IntCode {
        ...
        loop {
            ...
            match op {
                ADD => {
                    ...
                    self.ip += 1;
                    let rp = self.ops[self.ip] as usize;
                    self.ip += 1;
                ...
                INPUT => {
                    let input_storage_index = self.ops[self.ip] as usize;
                    self.ip += 1;
    ...
    fn resolve_op(&self, mode: OpMode) -> OpSize {
        match mode {
            ...
            OpMode::Positional => self.ops[self.ops[self.ip] as usize],
            OpMode::Relative => {
                let index = (self.ip as OpSize + self.relative_base) as usize;
                self.ops[self.ops[index] as usize]
            }
        }
    }
}

pub fn compute(ops: &mut Ops, input: &[OpSize]) -> Vec<OpSize> {
