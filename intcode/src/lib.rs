...
impl IntCode {
    ...
    pub fn compute_output(&mut self) -> Option<i32> {
        let ops = &mut self.ops;

        loop {
            let coded_op = ops[self.ip];
            ...
            match op {
                ADD => {
                    let a = resolve_op(ops, self.ip, mode1);
                    self.ip += 1;
                    let b = resolve_op(ops, self.ip, mode2);
                    self.ip += 1;
                    let rp = ops[self.ip] as usize;
                    self.ip += 1;
                ...
                INPUT => {
                    let input_storage_index = ops[self.ip] as usize;
                    ops[input_storage_index] = self.input[self.input_pos];
                    self.input_pos += 1;
                    self.ip += 1
                }
                OUTPUT => {
                    let input_index = ops[self.ip] as usize;
                    self.ip += 1;
                    let out = ops[input_index];
                    self.output.push(out);
                    return Some(out);
                }
                JUMP_IF_TRUE => {
